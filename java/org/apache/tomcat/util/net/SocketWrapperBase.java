...
public abstract class SocketWrapperBase<E> {
    ...
    protected int populateReadBuffer(ByteBuffer to) {
        // Is there enough data in the read buffer to satisfy this request?  （读缓冲区中是否有足够的数据可以满足此请求？）
        // Copy what data there is in the read buffer to the byte array      （将读缓冲区中的数据复制到byte数组）
        socketBufferHandler.configureReadBufferForRead();
        // 将from中的字节，转移到to中
        int nRead = transfer(socketBufferHandler.getReadBuffer(), to);

        if (log.isDebugEnabled()) {
            // debug打印从socket的buffer中，读了几个字节
    ...
    /**
     * 将from中position~limit中的字节，转移到to中。
     * 这里max的作用是防止溢出：看put()源码可以知道，如果from的剩余可读字节(即remaining)>to的剩余可写字节(即remaining)，会抛BufferOverflowException异常。
     *
     * 这里max表示，最多可以put多少，两种情况列举一下就懂了。主要防止第一种情况。
     * 1、from多，to少，max最多可以写to.remaining的字节。
     *    那么from的limit就应该是position + to.remaining
     * 2、from少，to多，这种情况就不解释了。
     *
     * @param from 源ByteBuffer
     * @param to   目标ByteBuffer
     * @return 转换成功几个字节
     */
    protected static int transfer(ByteBuffer from, ByteBuffer to) {
        ...
        if (max > 0) {
            ...
            // 重设from的limit，防止BufferOverflowException
