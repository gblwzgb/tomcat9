...
public class CoyoteAdapter implements Adapter {
    ...
    public void service(org.apache.coyote.Request req, org.apache.coyote.Response res)
        ...
        if (request == null) {
            // Create objects
            // 创建一个继承自HttpServletRequest的Request
            request = connector.createRequest();
            // 将CoyoteRequest记录都HttpServletRequest中
            request.setCoyoteRequest(req);
            // 创建一个继承自HttpServletResponse的Response
            response = connector.createResponse();
            // 将CoyoteResponse记录都HttpServletResponse中
            response.setCoyoteResponse(res);

            // Link objects
            // 让HttpServletRequest和HttpServletResponse互相持有引用
            ...
            // CoyoteRequest的queryString的编码为：UTF_8默认
     ...
     */
    /**
     * 在解析HTTP头之后，执行必要的处理，以使请求/响应pair可以传递到容器管道的开始处进行处理。
     */
