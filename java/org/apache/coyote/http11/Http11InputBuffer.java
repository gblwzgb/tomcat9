...
public class Http11InputBuffer implements InputBuffer, ApplicationBufferHandler {
    ...
    // 快照下一次开始读ByteBuffer的起始点(position)
     ...
     */
    // 最大允许大小，HTTP请求行 + header + 任意前导空白行，最大8K，来源于（org.apache.coyote.http11.AbstractHttp11Protocol.maxHttpHeaderSize）
     ...
     */
    /**
     * 读取请求行。该功能旨在解析HTTP请求头期间使用。不要尝试使用它读取请求体。
     *
     * @param keptAlive
     * @param connectionTimeout
     * @param keepAliveTimeout
     * @return 如果数据正确送入，则为true；否则为false。如果没有立即可用的数据并且应该释放线程，则返回false
     * @throws IOException
     */
    // 解析：https://blog.csdn.net/weixin_34203832/article/details/92295554
    boolean parseRequestLine(boolean keptAlive, int connectionTimeout, int keepAliveTimeout)
        ...
        if (parsingRequestLinePhase < 2) {
            do {
                // Read new bytes if needed  （根据需要读取新字节）
                if (byteBuffer.position() >= byteBuffer.limit()) {
                    if (keptAlive) {
                        // Haven't read any request data yet so use the keep-alive timeout.  (尚未读取任何请求数据，因此请使用保持活动超时)
                        wrapper.setReadTimeout(keepAliveTimeout);
                    }
                    if (!fill(false)) {
                        // 没有读到数据
                        // A read is pending, so no longer in initial state  （读取处于挂起状态，因此不再处于初始状态）
                        parsingRequestLinePhase = 1;
                        return false;
                    }
                    // At least one byte of the request has been received. Switch to the socket timeout.
                    // 至少收到了一个字节的请求。 切换到socket超时。
                ...
                if (!keptAlive && byteBuffer.position() == 0 && byteBuffer.limit() >= CLIENT_PREFACE_START.length - 1) {
                    ...
                    // 如果一字节不差的匹配到了CLIENT_PREFACE_START这个鬼东西，返回false
                    if (prefaceMatch) {
                ...
                // 一旦我们开始读取数据，就设置开始时间（即使只是跳过空白行）
                if (request.getStartTime() < 0) {
                    request.setStartTime(System.currentTimeMillis());
                }
                // 读出一个字节（内部position+1）
                chr = byteBuffer.get();
            } while ((chr == Constants.CR) || (chr == Constants.LF));  // 当读到/r或/n时（空白符），继续循环，直到读到一个非空白符时跳出while
            // 读取的开始位置往前移一格（因为在循环里get()的时候，chr被取出来导致position+1了）
            ...
            if (log.isDebugEnabled()) {
                // debug打印读取到的字节（跳过了\r和\n）
                // offset就是position，length就是remaining
                log.debug("Received ["
                        + new String(byteBuffer.array(), byteBuffer.position(), byteBuffer.remaining(), StandardCharsets.ISO_8859_1) + "]");
            }
        }
        /**
         * 阶段2：解析请求方法，POST 或 GET
         *
         * 从parsingRequestLineStart开始，一直找到下一个空白字符，两段空白之间的字节就表示请求方法
         */
        if (parsingRequestLinePhase == 2) {
            //
            ...
            while (!space) {
                ...
                // Spec说方法名称是一个令牌，后跟一个空格，但也可以容忍多个 空格 和/或 \r。
        ...
        /**
         * 阶段3：寻找定位到下一个非空白的位置，记录在parsingRequestLineStart上。
         */
        ...
        /**
         * 阶段4：解析URI，解析URI问号后面的queryString。
         */
        if (parsingRequestLinePhase == 4) {
            ...
            while (!space) {
                ...
                } else if (chr == Constants.QUESTION && parsingRequestLineQPos == -1) {  // 匹配到了"?"问号
        ...
        /**
         * 阶段5：寻找定位到下一个非空白的位置，记录在parsingRequestLineStart上。
         */
    ...
    boolean parseHeaders() throws IOException {
        ...
        do {
            ...
            /*
             * 检查：
             * （1）header加requestLine的大小不超过其限制
             * （2）读取body时，有足够的字节以避免扩展缓冲区
             * 技术上来说：（2）是技术限制，（1）是逻辑限制
             * 强制执行headerBufferSize含义的限制从如何分配buf和如何读取空白行的方式来看，仅检查（1）就足够了。
             */
            // 超过8K字节了还没读完，抛header too large异常
     ...
     *
     * @param block
     * @return 如果将更多数据添加到输入缓冲区，则为true，否则为false
     * @throws IOException
     */
    private boolean fill(boolean block) throws IOException {

        if (parsingHeader) {
            // 如果可读大于8*1024字节（8K），说明有问题，抛异常。
            // 也就是说Header是固定8K的。
            if (byteBuffer.limit() >= headerBufferSize) {
                if (parsingRequestLine) {
                    // Avoid unknown protocol triggering an additional error  （避免未知协议触发其他错误）
                    request.protocol().setString(Constants.HTTP_11);
                }
                // header太大异常。
                throw new IllegalArgumentException(sm.getString("iib.requestheadertoolarge.error"));
     ...
     * 解析一行HTTP header
     *
     * @return 读取空白行后为false（表示HTTP header解析完成）
     */
    private HeaderParseStatus parseHeader() throws IOException {
        ...
        while (headerParsePos == HeaderParsePosition.HEADER_START) {
            ...
            if (chr == Constants.CR && prevChr != Constants.CR) {
                // Possible start of CRLF - process the next byte.  (CRLF的可能开始-处理下一个字节。)
            } else if (prevChr == Constants.CR && chr == Constants.LF) {
                // 碰到了/r/n，header解析完毕
                ...
                if (prevChr == Constants.CR) {
                    ...
                    // 必须读取了两个字节（第一个是\r，第二个不是\n）
                    byteBuffer.position(byteBuffer.position() - 2);
                } else {
                    // Must have only read one byte (必须只读取一个字节)
                    byteBuffer.position(byteBuffer.position() - 1);
                }
                break;
            }
        ...
        if (headerParsePos == HeaderParsePosition.HEADER_START) {
            // Mark the current buffer position (mark当前缓冲区位置)
        ...
        //
        // Reading the header name          (读取header名称)
        // Header name is always US-ASCII   (header名称总是US-ASCII)
        //
        ...
        while (headerParsePos == HeaderParsePosition.HEADER_VALUE_START ||
            ...
            if (headerParsePos == HeaderParsePosition.HEADER_MULTI_LINE) {
                if ((peek != Constants.SP) && (peek != Constants.HT)) {
                    ...
                    // 在缓冲区中复制一个额外的空间（因为在行之间必须至少插入一个空间）
