...
public abstract class AbstractProtocol<S> implements ProtocolHandler,
    ...
    protected static class ConnectionHandler<S> implements AbstractEndpoint.Handler<S> {
        ...
        public SocketState process(SocketWrapperBase<S> wrapper, SocketEvent status) {
            ...
            // 获取已经关联了的 Processor，这个是处理半包问题用的
            ...
            try {
                ...
                if (state == SocketState.LONG) {
                    // In the middle of processing a request/response.   （在处理请求/响应的过程中。）
                    // Keep the socket associated with the processor.
                    // Exact requirements depend on type of long poll
                    // 在处理请求/响应的过程中。保持socket与processor的关联。确切要求取决于长轮询的类型

                    // 注册一个读兴趣事件，这个socket的下一个请求进来，会直接给这个socket相关联的processor继续解析。（粘包、拆包就是这么实现的）
                    ...
                    // 在保持活动状态但在请求之间。可以回收处理器。继续轮询下一个请求。
        ...
        protected void longPoll(SocketWrapperBase<?> socket, Processor processor) {
            if (!processor.isAsync()) {
                ...
                //  - the request line/headers have not been completely read
                // 当前仅用于HTTP
                // 要嘛
                // 1、这是升级的连接
                // 2、请求行/头尚未完全读完（拆包了）
